 ...
 * FIXED: Multi-URL failover for deployment resilience
 */

class CheeseBlockchainAPI {
    constructor(apiUrl, apiKey) {
        // Production blockchain server URLs (multiple for failover)
        // When one URL fails (e.g., after Railway redeploy), try the next
        this.productionUrls = [
            'https://ideal-quietude-production-22a8.up.railway.app',  // Railway auto-generated
            'https://cheese-blockchain.railway.app',                   // Custom domain (if configured)
            'https://cheese-blockchain-api.onrender.com',              // Render backup
        ];
        ...
        // Track current URL index for failover
        this.currentUrlIndex = 0;
        this.isLocalDev = isLocalDev;

        // Use provided URL, or detect environment
        if (apiUrl) {
            this.apiUrl = apiUrl;
        } else if (isLocalDev) {
            this.apiUrl = localDevUrl;
        } else {
            this.apiUrl = this.productionUrls[0];
        }
    ...
    // Try next URL in failover list
    tryNextUrl() {
        if (this.isLocalDev) return false; // Don't failover in local dev

        this.currentUrlIndex++;
        if (this.currentUrlIndex >= this.productionUrls.length) {
            this.currentUrlIndex = 0; // Reset to first URL
            return false; // All URLs tried
        }

        this.apiUrl = this.productionUrls[this.currentUrlIndex];
        console.log(`🔄 Trying failover URL: ${this.apiUrl}`);
        return true;
    }
    ...
    async request(endpoint, options = {}) {
        ...
        */
        ...
        try {
            ...
            // Provide more helpful error messages + FAILOVER
            if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
                if (this.tryNextUrl()) {
                    console.log('🔄 Connection failed, trying failover URL...');
                    return this.request(endpoint, options);
                }
                throw new Error('Cannot connect to blockchain server. All backup URLs tried.');
            }
            throw error;
        }
    }
